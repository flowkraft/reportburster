...
# Step 4: Pack docker folder as a zip file
rm -rf ./docker-temp/
mkdir ./docker-temp
...
# Step 5: Update docker-compose.yml and  with the version parsed previously
sed -i "s/reportburster_server:.*/reportburster_server:$version/" ./docker-temp/docker-compose.yml

# Update the startServer.sh startServer.sh with the latest version
sed -i "s/\(image=\"reportburster_server:\).*\"/\1$version\"/" ./docker-temp/startServer.sh
