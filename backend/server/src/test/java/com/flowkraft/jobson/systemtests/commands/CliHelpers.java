...
package com.flowkraft.jobson.systemtests.commands;
...
public final class CliHelpers {
	...
	public static CliOutputs runAndGetOutputs(File pwd, String... args) throws IOException, InterruptedException {
		final Process process = new ProcessBuilder(getAllArgs(Arrays.asList(args))).directory(pwd).start();

		/*
		 * BufferedReader readerStdOut = new BufferedReader(new
		 * InputStreamReader(process.getInputStream())); String line, stdoutLines=
		 * StringUtils.EMPTY, stderrLines = StringUtils.EMPTY; while ((line =
		 * readerStdOut.readLine()) != null) { stdoutLines = stdoutLines + line +
		 * StringUtils.LF; }
		 * 
		 * BufferedReader readerStdErr = new BufferedReader(new
		 * InputStreamReader(process.getErrorStream())); while ((line =
		 * readerStdErr.readLine()) != null) { stderrLines = stderrLines + line +
		 * StringUtils.LF; }
		 */
	...
	public static List<String> getAllArgs(List<String> extraArgs) {
		...
		processArgs.add(ServerApplication.class.getCanonicalName());
