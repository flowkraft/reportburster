...
public class ExcelReporterTest {
	...
	private static final String PAYSLIPS_DOCX_EXCEL_TEMPLATE_PATH = "src/main/external-resources/template/samples/reports/payslips/payslips-template-excel.docx";
	...
	public final void generateReportsFromExcelWithMergedCellsComplex() throws Exception {
		...
		List<String[]> arrayData = TestsUtils.toArrayRows(reporter.getCtx().reportData);

		assertEquals("parsedLines.size() should be 5", 5, arrayData.size());
		...
		assertEquals("First row data column 1", arrayData.get(0)[0]);
	...
	public final void generateReportsFromExcelWithTablesComplex() throws Exception {
		...
		assertEquals("Data in table should be extracted", "Table Row 1", arrayData.get(0)[0]);

		for (int i = 0; i < arrayData.size(); i++) {
			String expectedToken = arrayData.get(i)[0];
			File outputFile = new File(reporter.getCtx().outputFolder + "/" + expectedToken + ".pdf");
			assertTrue("Output file should exist: " + outputFile.getPath(), outputFile.exists());
		}
	}
	...
	public final void generateReportsWithCombinedOptions() throws Exception {
		...
		assertEquals("parsedLines.size() must be 6", 6, arrayData.size());
		...
		assertFalse("First parsed row should not be header content",
				arrayData.get(0)[0].toLowerCase().contains("column"));
	...
	public final void generateDocxReports() throws Exception {
		ExcelReporter reporter = new TestBursterFactory.ExcelReporter(StringUtils.EMPTY,
			...
			protected void executeController() throws Exception {
				...
				ctx.settings.getReportTemplate().documentpath = PAYSLIPS_DOCX_EXCEL_TEMPLATE_PATH;
			}
		};
	...
	public final void generateReportsFromHeaderFirstLine() throws Exception {
		...
		assertEquals("Data Row 1", arrayData.get(0)[0]);
	}
	...
	public final void generateReportsFromHeaderNoHeader() throws Exception {
		...
		assertEquals("Column 1", arrayData.get(0)[0]);
	}
	...
	public final void generateReportsFromHeaderMultiLine() throws Exception {
		...
		assertNotNull(arrayData);
		assertEquals("parsedLines.size() should be 4", 4, arrayData.size());

		// First row should be the third row in the file
		assertEquals("Data Row 2", arrayData.get(0)[0]);
	}
	...
	public final void generateReportsFromUsingLastColumnAsId() throws Exception {
		...
		// Get the parsed lines to know what values to expect in filenames
		List<String[]> arrayData = TestsUtils.toArrayRows(reporter.getCtx().reportData);
		assertEquals("parsedLines.size() should be 3", 3, arrayData.size());
	...
	public final void generateReportsFromUsingCustomColumnIndexAsId() throws Exception {
		...
		// Get the parsed lines to know what values to expect in filenames
		List<String[]> arrayData = TestsUtils.toArrayRows(reporter.getCtx().reportData);
		assertEquals("Sheet should have 3 rows", 3, arrayData.size());
	...
	public final void generateReportsFromFormulasUseResults() throws Exception {
		...
		String formulaResult = arrayData.get(0)[2];
	...
	public final void generateReportsFromFormulasUseFormulas() throws Exception {
		...
		String formulaText = arrayData.get(5)[2];
	...
	public final void generateReportsFromCustomSheetIndex() throws Exception {
		...
		assertEquals("Sheet2", arrayData.get(0)[0]);
	}
	...
	public final void generateReportsFromPreserveLeadingWhitespace() throws Exception {
		...
		assertTrue("Cell should NOT have leading whitespace", !arrayData.get(4)[1].startsWith(" "));
		assertTrue("Cell should have leading whitespace", arrayData.get(5)[1].startsWith(" "));
	}
	...
	public final void generateReportsFromMultiSheetWithVaryingWidths() throws Exception {
		...
		List<String[]> arrayDataSheet1 = TestsUtils.toArrayRows(reporter.getCtx().reportData);
		assertNotNull("Parsed lines for sheet 1 should not be null", arrayDataSheet1);
		assertEquals("Sheet 1 should have 3 rows", 3, arrayDataSheet1.size());

		// Verify column count for first sheet
		assertEquals("Sheet 1 should have 5 columns", 5, arrayDataSheet1.get(0).length);

		// Verify last column is used as ID for files
		for (String[] row : arrayDataSheet1) {
		...
		List<String[]> arrayDataSheet2 = TestsUtils.toArrayRows(reporter2.getCtx().reportData);
		assertNotNull("Parsed lines for sheet 2 should not be null", arrayDataSheet2);
		assertTrue("Should have parsed at least one data row from sheet 2", arrayDataSheet2.size() > 0);
		assertEquals("Sheet 2 should have 3 rows", 3, arrayDataSheet2.size());

		// Verify column count for second sheet
		assertEquals("Sheet 2 should have 3 columns", 3, arrayDataSheet2.get(0).length);

		// Verify last column is used as ID for files, though sheet has fewer columns
		for (String[] row : arrayDataSheet2) {
