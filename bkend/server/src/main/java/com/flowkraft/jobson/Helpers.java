...
import static com.flowkraft.jobson.Constants.STDIO_BUFFER_LEN_IN_BYTES;
...
public final class Helpers {
	...
	private static final char[] base36Characters = new char[] { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k',
	...
	public static <T> Optional<T> lastElement(List<T> els) {
		if (els.size() == 0)
			...
			return Optional.of(els.get(els.size() - 1));
	}
	...
	public static <T, U, V> Map<V, U> mapKeys(Map<T, U> m, Function<T, V> f) {
		return m.entrySet().stream().map(e -> new AbstractMap.SimpleEntry<>(f.apply(e.getKey()), e.getValue()))
				.collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
	}

	public static <T, U, V> Map<T, V> mapValues(Map<T, U> m, Function<U, V> f) {
		return m.entrySet().stream().map(e -> new AbstractMap.SimpleEntry<>(e.getKey(), f.apply(e.getValue())))
				.collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
	}
	...
	public static void attachTo(Process process, Observer<byte[]> stdoutObserver, Observer<byte[]> stderrObserver,
