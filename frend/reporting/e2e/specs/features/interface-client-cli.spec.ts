...
/**
 * CLI Interface Tests — verifies reportburster.bat works for every operation.
 *
...
test.describe('CLI — System Commands', () => {
  ...
  test('--help shows usage', async () => {
    ...
    expect(result.exitCode).toEqual(0);
    // reportburster.bat redirects Java output to logs/reportburster.bat.log
    const batLog = fs.readFileSync(path.join(PORTABLE_DIR, 'logs/reportburster.bat.log'), 'utf-8');
    expect(batLog).toContain('reportburster');
  });
