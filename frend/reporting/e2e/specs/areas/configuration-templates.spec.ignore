...
test.describe('', async () => {
  electronBeforeAfterAllTest(
    ...
    async function ({ beforeAfterEach: firstPage }) {
      ...
      ft = ConfTemplatesTestHelper.assertFallbackTemplate(ft, 'Bursting');
      ...
      return ConfTemplatesTestHelper.assertFallbackTemplate(ft, 'Bursting');
    },
  );
  ...
  electronBeforeAfterAllTest(
    'should verify that duplicate action works correctly',
    async function ({ beforeAfterEach: firstPage }) {
      //long running test
      test.setTimeout(Constants.DELAY_FIVE_THOUSANDS_SECONDS);
