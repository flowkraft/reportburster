 ...
 *
 * 2.	curl is a tool to transfer data from or to a server, using one 
 * 		of the supported protocols (DICT, FILE, FTP, FTPS, GOPHER, HTTP,
 ...
 *
 * 4.	The URL syntax is protocol-dependent. You'll find a detailed
 ...
 *		script.
 *
 * 7.	For a full documentation of the cURL and SFTP please see
 *			
...
/*
 *		By default the script is extracting the required SFTP session
 ...
 *			If there is no file part in the specified URL, Curl will append
 *			the local file name. 
 ...
 *			to use. That will most likely cause the upload operation to fail. 
 *			If this is used on a HTTP(S) server, the PUT command will be used. 
 *
 ...
 *				part and only shows the	ASCII part of the dump.
 *				It makes smaller output that might be easier to read for
 *				untrained humans.	
 ...
 *				Mostly useful for debugging. A line starting with '>' means "header data" 
 *				sent by curl, '<' means "header data" received by curl that is hidden
 *				in normal cases, and a line starting with '*' means additional info
 *				provided by curl.
 ...
 *				Note that the private key may itself be protected by a
 *				password that is unrelated to the login password of the remote system.
