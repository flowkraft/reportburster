...
const _splitVerifyBigFile = (
  ...
  if (pauseCancelMode.includes('after-pdf-generated')) {
    ...
    if (
      ...
      //ft.click('#btnCheckLicenseKey');
      ft.click('#btnActivateLicenseKey');
    else if (pauseCancelMode === 'pause-after-pdf-generated-and-then-resume')
      ft.click('#btnActivateLicenseKey');
  ...
  ft.appShouldBeReadyToRunNewJobs()
    .appStatusShouldBeGreatNoErrorsNoWarnings()
    .click('#burstFile')
    .typeText(
      path.resolve(
        slash(
          process.env.PORTABLE_EXECUTABLE_DIR +
            '/samples/burst/Payslips-Oct.pdf'
        )
      )
    );
  ...
  ft.click('#btnBurst').clickYesDoThis();

  if (pauseCancelMode.includes('after-pdf-generated')) {
    ft.click('#btnClearLogs').clickYesDoThis();
    ft.waitOnElementToBecomeVisible('#qaReminderLink');
  }

  ft.elementShouldNotBeVisible('#logsViewer')
    .click('#btnBurst')
    .clickYesDoThis()
