export const tabDocumentationTemplate = `<ng-template #tabDocumentationTemplate>
  <div class="well">
    <br />

    <ol>
      <li>
        <a href="https://www.reportburster.com/docs/quickstart" target="_blank">QuickStart -
          <em>ReportBurster</em> in 5 Minutes</a> will help you to quickly get started with report bursting
      </li>
      <br>
      <li>
        <a href="https://www.pdfburst.com/docs/html/userguide/index.html" target="_blank">
          <em>ReportBurster</em> User Guide</a> explains with more details what &amp; how can be achieved with
        <em>ReportBurster</em></li>
      <br>
      <li>
        <a id='checkPointHelpDocumentation' href="https://www.pdfburst.com/docs/html/advanced/index.html" target="_blank">
          <em>ReportBurster</em> - Advanced Report Delivery Scenarios</a> explains how
        <em>ReportBurster</em> scripting can be used to achieve more complex report delivery scenarios</li>
    </ol>
  </div>
</ng-template>`;
