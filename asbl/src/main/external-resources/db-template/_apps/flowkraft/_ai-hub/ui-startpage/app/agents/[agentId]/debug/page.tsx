...
function AccordionTab({ 
  ...
  return (
    <div className="border border-border rounded">
      <button 
        onClick={onToggle}
        className="w-full px-3 py-2 text-left bg-muted hover:bg-muted flex items-center justify-between text-sm"
      >
        {header}
  ...
  return (
    <div className="min-h-screen bg-muted p-6">
      <div className="max-w-[1800px] mx-auto">
        {/* Header */}
        <div className="mb-6">
          <Link href="/agents" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground mb-2">
            <ChevronLeft className="w-4 h-4 mr-1" />
            Back to Agents
          </Link>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Brain className="w-6 h-6 text-athena-accent" />
            {agent ? `${agent.name ?? ''} (${(agent.metadata?.agentKey) ?? agent.id})` : 'Loading...'}
            {agent?.enable_sleeptime && (
              <span className="ml-2 text-sm font-normal text-muted-foreground" title="Sleeptime enabled">
                💤 Sleeptime
              </span>
            )}
          </h1>
        ...
        {loading && <div className="text-muted-foreground">Loading agent details...</div>}
        ...
        {!loading && agent && (
          <>
            <div className="grid grid-cols-12 gap-4">
              ...
              <div className="col-span-3 bg-card border border-border rounded-lg p-4 space-y-4">
                {/* Model */}
                <div>
                  ...
                  <div className="text-sm bg-muted px-2 py-1 rounded">{agent.model}</div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-xs bg-muted p-2 rounded max-h-40 overflow-auto whitespace-pre-wrap font-mono">
                    {agent.system || <em className="text-muted-foreground">No system instructions</em>}
                  </div>
                </div>

                {/* Tags */}
                <div>
                  <div className="text-sm font-semibold text-foreground mb-1">Tags</div>
                  <div className="flex flex-wrap gap-1">
                    {(() => {
                      ...
                      if (!t || t.length === 0) return <span className="text-xs text-muted-foreground">No tags</span>;
                      return t.map((tg: any) => (
                        <span key={String(tg)} className="text-xs bg-muted border border-border px-2 py-0.5 rounded-full">
                          {tg}
                        </span>
                      ));
                    })()}
                      ...
                      <>
                        ...
                        <div>
                          ...
                          {memoryBlockTools.length === 0 ? (
                            <div className="text-muted-foreground ml-4">none</div>
                          ) : (
                        ...
                        <div>
                          ...
                          {customTools.length === 0 ? (
                            ...
                            <>
                              ...
                              {selectedTool && (
                                <div className="mt-2 border border-border rounded p-2 bg-muted">
                                  <div className="font-semibold mb-1">
                                    {selectedTool.name} - Source
                                    {loadingToolSource && <span className="ml-2 text-muted-foreground">Loading...</span>}
                                  </div>
                                  <textarea
                                    readOnly
                                    value={toolSourceCode}
                                    className="w-full h-48 font-mono text-[10px] p-2 border border-border rounded bg-card resize-y"
                                  />
                                </div>
                              )}
                            </>
                        ...
                        {otherTools.length > 0 && (
                          <div>
                            <div className="font-semibold text-muted-foreground mb-1">Other ({otherTools.length})</div>
                            <ul className="ml-4 space-y-0.5">
                ...
                {activeToolsTab === 'sourcesTab' && (
                  <div className="text-xs">
                    {(agent.sources || []).length === 0 ? (
                      <div className="text-muted-foreground">No sources</div>
                    ) : (
              ...
              <div className="col-span-5 bg-card border border-border rounded-lg p-4">
                ...
                <div className="border border-border rounded p-3 h-[420px] overflow-auto bg-muted">
                  <div className="space-y-3">
                    {messages.map((m) => (
                      <div key={m.id} className={`p-2 rounded ${m.role === 'user' ? 'bg-primary/10 ml-8' : 'bg-card border border-border mr-8'}`}>
                    ...
                    {(status === 'submitted' || status === 'streaming') && (
                      <div className="text-muted-foreground text-sm">💬 Thinking{dots}</div>
                    )}
                ...
                <form onSubmit={onSubmit} className="mt-3 flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 border border-border rounded text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  />
              ...
              <div className="col-span-4 bg-card border border-border rounded-lg p-4">
                <h3 className="text-sm font-semibold text-foreground mb-3">Context Window</h3>
                ...
                <div className="mb-4">
                  <div className="text-xs font-semibold text-muted-foreground mb-2">Memory Blocks</div>
                  {(agent.memory?.blocks || []).length === 0 ? (
                    <div className="text-xs text-muted-foreground">No core memory</div>
                  ) : (
                ...
                {/* Archives */}
                <div>
                  <div className="text-xs font-semibold text-muted-foreground mb-2">Archival Memories</div>
                  <input
                    placeholder="Search archives..."
                    value={archiveQuery}
                    onChange={(e) => setArchiveQuery(e.target.value)}
                    className="w-full px-2 py-1 text-xs border border-border rounded mb-2"
                  />
                  {(() => {
                    const filtered = (agent.archives || []).filter((a: any) => 
                      String(a.name || '').toLowerCase().includes(archiveQuery.toLowerCase())
                    );
                    if (filtered.length === 0) return <div className="text-xs text-muted-foreground">No archives</div>;
                    return (
            ...
            {agent.enable_sleeptime && (
              <div className="mt-4 bg-card border border-border rounded-lg p-4">
                ...
                {sleeptimeLoading && <div className="text-muted-foreground text-sm">Loading sleeptime details...</div>}
                {sleeptimeError && <div className="text-red-500 text-sm">Error: {sleeptimeError}</div>}
                {!sleeptimeLoading && !sleeptimeError && !sleeptimeAgent && (
                  <div className="text-muted-foreground text-sm">No sleeptime agent found sharing memory with this agent.</div>
                )}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div>
                      <div className="text-sm font-medium mb-2">
                        ...
                        {sleeptimeAgent.metadata?.agentKey && (
                          <span className="text-muted-foreground font-normal"> ({sleeptimeAgent.metadata.agentKey})</span>
                        )}
                      </div>
                      
                      <div className="text-xs font-semibold text-muted-foreground mb-1">Attached Archives</div>
                      {(!sleeptimeAgent.archives || sleeptimeAgent.archives.length === 0) ? (
                        <div className="text-xs text-muted-foreground">No archives</div>
                      ) : (
                        <ul className="text-xs space-y-0.5">
                    ...
                    <div>
                      ...
                      {(!sleeptimeAgent.memoryBlocks || sleeptimeAgent.memoryBlocks.length === 0) ? (
                        <div className="text-xs text-muted-foreground">No memory blocks</div>
                      ) : (
                        <div className="space-y-1">
                    ...
                    <div>
                      ...
                      {(!sleeptimeAgent.tools || sleeptimeAgent.tools.length === 0) ? (
                        <div className="text-xs text-muted-foreground">No tools attached</div>
                      ) : (
                        <ul className="text-xs space-y-0.5">
