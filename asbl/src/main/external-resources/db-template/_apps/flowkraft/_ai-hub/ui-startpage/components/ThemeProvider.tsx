...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function loadThemeFromDB() {
      try {
        // Fetch both theme settings in parallel from SQLite via API
        const [colorRes, modeRes] = await Promise.all([
          fetch(`/api/config?key=${encodeURIComponent(THEME_COLOR_KEY)}`),
          fetch(`/api/config?key=${encodeURIComponent(THEME_MODE_KEY)}`),
        ])

        const colorData = colorRes.ok ? await colorRes.json() : null
        const modeData = modeRes.ok ? await modeRes.json() : null

        const savedTheme = colorData?.config?.value || DEFAULT_THEME
        const savedMode = modeData?.config?.value || DEFAULT_MODE
