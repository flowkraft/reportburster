#+TITLE: HR Payslips Portal — Admin Panel Tasks (WordPress + PODS)
...
- Enable REST API: Yes (needed for DataPallas publishing)
...
- Store the page ID in WordPress option: ~DataPallas_account_page_id~
...
- This endpoint is used by the DataPallas Groovy script to publish payslips
