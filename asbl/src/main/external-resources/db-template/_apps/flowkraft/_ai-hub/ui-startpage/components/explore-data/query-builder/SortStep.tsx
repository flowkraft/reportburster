  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center gap-2">
        ...
        <button id="btnAddSort" onClick={addSort} className="p-0.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground">
      ...
      {sort.map((s, i) => (
        <div key={i} className="flex items-center gap-1.5 ml-6">
          ...
          <button id={`btnRemoveSort-${i}`} onClick={() => removeSort(i)} className="p-0.5 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive">
            <X className="w-3 h-3" />
          </button>
        </div>
      ))}
