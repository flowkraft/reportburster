...
**Keep it simple:** Most [report bursting](https://www.reportburster.com/docs/report-bursting) tasks can be achieved without any scripting. Assume scripting is not needed unless the user's requirements clearly call for it (conditional distribution, custom uploads, etc.) — but stay open that in some advanced cases it might actually help. Just don't push users toward scripting directly.
