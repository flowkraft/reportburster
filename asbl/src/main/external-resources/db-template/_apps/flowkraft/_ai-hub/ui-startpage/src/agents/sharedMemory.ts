...
function getSkill(name: string): { name: string; description: string } {
    const skillDescriptions: Record<string, string> = {
        ...
        'reportburster-ui-and-docs-navigation': 'Complete map of the ReportBurster application UI and documentation site. Use when a user asks "where do I find X?", "how do I get to Y?", "where is the docs page for Z?", or needs help navigating menus or finding the right documentation. Covers: top navigation bar (Processing, Configuration, Help & Support), left sidebar menus for each area, what each screen does, and the full documentation site map with verified URLs for every topic. Key docs: https://www.reportburster.com/docs',
        'reportburster-cookbook': 'Gold mine of working examples, sample configs, and AI prompt templates. Most report bursting tasks can be achieved without scripting — assume scripting is not needed unless the requirements clearly call for it, but stay open that in advanced cases it might help. Use when users need to build reports, email templates, dashboards, charts, pivot tables, or Groovy scripts — study existing samples first, then adapt. Covers: report bursting & generation samples in /reportburster/config/samples/, report template gallery, BI/analytics sample configs in /reportburster/config/samples/_frend/, Groovy script samples, and an AI prompt reference library (DB schemas, email/HTML/PDF/Excel templates, SQL, scripting). Read SKILL.md for the full catalog, then dive into specific files as needed.',
