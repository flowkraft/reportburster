...
function getSkill(name: string): { name: string; description: string } {
    const skillDescriptions: Record<string, string> = {
        ...
        'datapallas-quickstart-setup-installation': 'Guide users through DataPallas installation and first-run experience. Prerequisites: Java 17+ (Chocolatey helps install it on Windows), Docker for extra apps. I help users download, extract, launch DataPallas, and burst the sample Payslips.pdf to see immediate results. I know the "Run as Administrator" flow for Java/Chocolatey installation. Key docs: https://datapallas.com/docs/quickstart',
        'datapallas-configuration': 'Guide users through DataPallas configuration via the UI. I cover two areas: (1) Core Settings — burst file naming patterns, output/quarantine folders, delivery options (email, upload, SMS, web), delimiters, and advanced settings; (2) Configuration Templates — creating report-specific configurations for different document types (Invoices, Bills, Statements) via Configuration → Manage → Reports menu. Each template gets its own email subject, burst file name, and output folder while sharing common settings. I always navigate users through the UI menus — I never edit settings.xml or other XML files directly. Key docs: https://datapallas.com/docs/configuration',
        'datapallas-report-bursting': 'Expert in report bursting — splitting PDF or Excel documents into multiple files based on burst tokens. For PDFs: tokens are text wrapped in curly brackets {like_this}. For Excel: two methods — (1) Burst by Distinct Sheets (sheet name = token), (2) Burst by Distinct Column Values (requires burst metadata sheet). I understand token placement, custom output filenames (${burst_token}), merging before bursting, and common pitfalls (missing tokens, wrong delimiters, invisible white-font tokens). Works with reports from any source: Crystal Reports, SAP, Oracle, Microsoft, etc. Key docs: https://datapallas.com/docs/report-bursting',
        'datapallas-report-distribution': 'Expert in automated report distribution — two main channels: (1) Email with SMTP configuration, well-known provider presets (Gmail, Outlook/Office 365), personalized subjects/bodies using variables (${var0}, ${burst_token}); (2) Upload/Archive via FTP, FTPS, SFTP/SSH/SCP, HTTP/WebDAV, Windows File Share — powered by cURL integration. I help configure output folders with dynamic variables (${input_document_name}, ${now?string["yyyy-MM"]}), set up archive patterns (by quarter, customer, department), and troubleshoot delivery issues. Key docs: https://datapallas.com/docs/report-distribution-email | https://datapallas.com/docs/report-distribution-upload',
        'datapallas-variables': 'Expert in DataPallas variables and templating — dynamic values extracted from reports at runtime. Two types: (1) Built-in variables like ${burst_token}, ${input_document_name}, ${input_document_extension}, ${now?string["format"]}, ${burst_index}, ${extracted_file_path}; (2) User-defined variables ${var0} through ${var19} populated via <0>value</0> tags in PDFs or userVariables column in Excel burst metadata sheet. Variables personalize: file names, output folders, email subjects/bodies, upload paths. I help users place variable tags (white font trick for invisibility), configure dynamic patterns, and troubleshoot variable extraction. Key docs: https://datapallas.com/docs/variables',
        'datapallas-quality-assurance': 'Expert in QA and testing for report distribution — validating configurations before going live. I help with: (1) Test Email Server (localhost:8025) — captures test emails without actual delivery, inspect attachments and content; (2) Distribution Testing — test all tokens, specific tokens, or random sample; (3) Error Handling — stop on first failure vs continue, retry policy with exponential backoff (delay, max delay, max retries); (4) Quarantine Management — failed reports quarantined for review and manual redistribution; (5) Logging & Tracing — INFO/ERROR/WARN logs, job monitoring, green/red status indicators. Key docs: https://datapallas.com/docs/report-distribution-qa',
        'datapallas-database-connections': 'Expert in database connections for report generation. I guide users through Configuration → Connections (Email, Databases) to set up connections to PostgreSQL, MySQL, SQL Server, Oracle, or any JDBC-compatible database. DataPallas auto-fetches schema after connection. **For my own understanding**, I actively read connection folder files in config/connections/<connection-name>/: (1) XML connection file — JDBC URL, host, port, credentials; (2) *-information-schema.json — raw database schema; (3) *-domain-grouped-schema.json — tables by business domain; (4) optional: *.puml ER diagrams, *-ubiquitous-language.txt glossary. **Warning: these files can be huge** — I grep/search for specific tables/columns rather than reading entire files at once. Key docs: https://datapallas.com/docs/data-exploration/database-connections',
        'datapallas-semantic-layer-cubes': 'Expert in the DataPallas Semantic Layer — reusable **Cube** definitions that sit between raw database tables and data consumers (Canvas, dashboards, reports). A Cube defines: **dimensions** (columns to group/filter by, with business-friendly names), **measures** (aggregated values such as SUM, COUNT, AVG), and **joins** (pre-wired table relationships so users never write JOIN SQL by hand). Why cubes matter: drop a cube on the Explore Data Canvas and get live results immediately with no schema knowledge required; the same cube powers Canvas widgets, dashboards, and reports from one definition; business names ("Total Revenue") replace SQL ("SUM(order_details.unit_price * quantity)") for non-technical users. Navigation: top menu **Configuration → Reports, Connections & Cubes**, then left menu **Cubes / Semantic Layer**. Key docs: https://datapallas.com/docs/semantic-layer',
        'datapallas-data-exploration': 'Expert in the DataPallas **Explore Data Canvas** — a drag-and-drop visual workspace for ad-hoc data exploration beyond pre-configured reports. Where it lives: top menu **Processing**, then the **Explore Data & Build Dashboards** tab. Workflow: pick a database connection → drag a Cube or table onto the canvas → tick fields → pick a visualization (Table, Chart, Pivot, Map, Number/Gauge/Progress, Trend, Detail, Sankey, Filter Pane) → refine with Filter / Group By / Summarize / Sort. Two modes: (1) **Visual mode** — no SQL needed; smart defaults choose a sensible chart type from the data shape; (2) **Finetune mode** — drop into raw SQL or a Groovy script when the UI cannot express the needed logic; the **"Hey AI, Help Me…"** button drafts SQL/script against the live schema. Multiple widgets sit side-by-side on the same canvas; a shared Filter Pane at the top refreshes all widgets at once. Cubes shine here — dimensions and joins already wired, no foreign keys to remember. A canvas can be **published** to create a shareable dashboard Report. Data privacy: AI works with schema/metadata only, not actual data rows. Key docs: https://datapallas.com/docs/data-exploration | https://datapallas.com/docs/data-exploration/canvas',
        'datapallas-report-generation': 'Expert in report generation from data sources. I help users configure: (1) Data Sources — SQL queries, Groovy scripts, or file sources (CSV, TSV, Excel, Fixed-Width); (2) Output Formats — PDF (HTML→PDF or XSL-FO), Excel (xlsx), HTML, Word (docx), XML/JSON; (3) Templates — HTML/CSS templates, template gallery, or custom designs. I also help with report parameters (runtime prompts) and data transformations (Groovy scripts for filtering, grouping, enrichment). Menu: Configuration → <Report Name> → Report Generation. Key docs: https://datapallas.com/docs/report-generation',
        'datapallas-dashboards': 'Expert in building DataPallas BI dashboards that combine charts, pivot tables, data tables, and KPI cards. **Two equivalent paths to the same output files:** (1) **Approach 1 — Data Canvas (visual-first):** open **Processing → Explore Data & Build Dashboards** tab → drop cubes/tables → configure widgets visually (Table, Chart, Pivot, Number/KPI, Trend, Gauge, Map, Sankey) → publish to create a Report. The canvas auto-exports the data script, HTML template, and per-component configs. (2) **Approach 2 — Fully Configure:** write a Groovy data script using ctx.dbSql.rows(sql) and ctx.reportData(\'componentId\', data); design an HTML template with <rb-value>, <rb-chart>, <rb-tabulator>, <rb-pivot-table>, <rb-parameters> web components; configure each component with a small Groovy DSL block. Both approaches produce identical files on disk. Key scripting patterns: componentId guard (if (!componentId || componentId == \'widgetName\')), parameter access (ctx.variables.getUserVariables(ctx.token ?: \'\')), multi-component optimization (multiple <rb-value> sharing one component-id = one HTTP request, fields pick columns from the cached result). Working sample: /datapallas/config/samples/_frend/dashboard-cfo/. Key docs: https://datapallas.com/docs/bi-analytics/dashboards',
        'datapallas-scripting': 'Expert in DataPallas scripting and integration. Three topics: (1) **Groovy lifecycle hooks** — hook into burst events (startBursting, startExtractDocument, endExtractDocument, startDistributeDocument, endDistributeDocument, quarantineDocument, endBursting) for custom logic; access burst context (ctx.token, ctx.extractedFilePath, ctx.variables, ctx.settings, ctx.skipCurrentFileDistribution); sample scripts at scripts/burst/samples/ cover zip, encrypt, print, FTP/SFTP/HTTP via cURL, fetch distribution details from DB/CSV, conditional skip, page-numbering, cloud sync. (2) **CLI** — every core operation (burst, generate, merge, resume, document, jasper, service, system) is exposed on the command line for shell scripts and schedulers; ready-to-run examples at https://datapallas.com/docs/advanced/cli. (3) **REST API** — same operations are also exposed as REST endpoints across domains (jobs, reports, connections, analytics, queries, starter-packs, system, license) for integrating DataPallas into workflows, CI/CD pipelines, or custom applications via plain HTTP; interactive docs at http://localhost:9090/swagger-ui.html when the server is running; written examples at https://datapallas.com/docs/advanced/api. **Note:** dashboard data-fetching scripts (ctx.dbSql.rows(sql), ctx.reportData(\'id\', rows)) are dashboard-specific — they live in the `datapallas-dashboards` skill. Key docs: https://datapallas.com/docs/advanced/scripting | https://datapallas.com/docs/advanced/cli | https://datapallas.com/docs/advanced/api',
        'datapallas-self-service-document-web-portal': 'Expert in custom web portals for two use cases: (1) Self-Service Document Portals — secure access for employees, customers, partners, students to view their own documents (payslips, invoices, statements); (2) Custom Analytics Dashboards — OLAP-driven portals with charts, KPIs, pivot tables, data visualizations. Portal stack options: WordPress+PODS (bundled), Grails (recommended), NextJS/React/Tailwind. Document publishing via DataPallas-scripting (endExtractDocument hook) + REST API. Features: user management, auto-provisioning, notifications (SMS/email). Boilerplate: _apps/cms-webportal-playground (WordPress), _apps/flowkraft/grails-playground, _apps/flowkraft/next-playground. Key docs: https://datapallas.com/docs/document-portal | https://datapallas.com/docs/document-portal/development-stacks',
        'datapallas-server': 'Expert in DataPallas Server — centralized, unattended report processing for enterprise environments. Key capabilities: (1) Server Mode — runs as Windows service with automatic startup, web interface at http://machine-name:9090, supports multiple users; (2) Automatic Polling — drop reports into poll/ folder for automatic processing; (3) Scheduling — integrate with Windows Task Scheduler or enterprise systems for nightly/weekly/monthly jobs; (4) Management — startServer.bat, shutServer.bat, service.bat install/uninstall. Same UI and configuration as desktop version, but designed for 24/7 unattended operation. Key docs: https://datapallas.com/docs/server | https://datapallas.com/docs/server/installation | https://datapallas.com/docs/server/scheduling',
        'datapallas-ui-and-docs-navigation': 'Complete map of the current DataPallas application UI. Use when a user asks "where do I find X?", "how do I get to Y?", or needs help navigating menus. **Top menu bar (3 entries):** **Processing** (direct link) · **Configuration ▾** (dropdown) · **Help & Support ▾** (dropdown). **Configuration dropdown** lists the fallback configuration template ("Bursting" by default) followed by a single link **"Reports, Connections & Cubes"** that opens the management area. **Reports, Connections & Cubes area — left menu (3 items):** **Reports** (paginated table of all configured reports) · **Connections** (email + database profiles) · **Cubes / Semantic Layer** (define dimensions, measures, joins over your tables). **Processing area — left menu:** Processing (Burst) · Merge → Burst · Quality Assurance · Logging, Tracing · Samples. When **Processing (Burst)** is selected, the main pane shows tabs: **Burst Reports** · **Generate Reports** · **Explore Data & Build Dashboards** · Logging and Tracing · License. The Explore Data & Build Dashboards tab hosts the visual Canvas for ad-hoc analysis and dashboard prototyping. **Per-template Configuration area — left menu** (after picking a configuration from the Configuration dropdown): General · Reporting · Enable/Disable Delivery · Email (+ Cloud Email Providers) · Upload · Documents2Web · SMS (+ Twilio) · Quality Assurance (+ Test Email Server) · Advanced (+ Error Handling) · Reports. **Help & Support dropdown:** Get Software Support · DocumentBurster Services · Apps / Starter Packs · Documentation · Examples · Customer Reviews · DocumentBurster Blog · Install/Setup (Electron) · License · About. When guiding users I say things like: "In the top menu, click **Configuration → Reports, Connections & Cubes**, then in the left menu click **Cubes / Semantic Layer**" or "In the top menu click **Processing**, then open the **Explore Data & Build Dashboards** tab". Key docs: https://datapallas.com/docs',
        'datapallas-cookbook': 'Gold mine of working examples, sample configs, and AI prompt templates. Most report bursting tasks can be achieved without scripting — assume scripting is not needed unless the requirements clearly call for it, but stay open that in advanced cases it might help. Use when users need to build reports, email templates, dashboards, charts, pivot tables, or Groovy scripts — study existing samples first, then adapt. Covers: report bursting & generation samples in /datapallas/config/samples/, report template gallery, BI/analytics sample configs in /datapallas/config/samples/_frend/, Groovy script samples, and an AI prompt reference library (DB schemas, email/HTML/PDF/Excel templates, SQL, scripting). Read SKILL.md for the full catalog, then dive into specific files as needed.',
        'datapallas-troubleshooting': 'Expert troubleshooter for DataPallas issues. My diagnostic flow: (1) Check /logs/errors.log for Java stacktraces; (2) Check /logs/datapallas.bat.log for command context; (3) Compare config vs /config/_defaults/settings.xml; (4) Check /config/samples for working examples. Top issues: "Emails not sending" — 90% forgot to enable Send documents by Email checkbox (OFF by default); "Bursting not working" — 90% missing/misconfigured burst tokens in source document. Java stacktraces almost NEVER mean DataPallas bugs — they indicate misconfiguration, bad input data, or user changes that broke things. For startup issues, check readme-Prerequisites.txt. Key docs: https://datapallas.com/docs/troubleshooting',
        
        // Data modeling skills (Athena)
        'sql-queries-plain-english-queries-expert': 'SQL expert who helps users build, optimize, and fix database queries. I help with: (1) Building SQL from requirements ("I need a query that joins X with Y..."); (2) Translating plain English to SQL ("Show top 10 customers"); (3) Optimizing slow queries; (4) Fixing syntax errors. **SQL Dialect Matters:** Oracle ≠ SQL Server ≠ MySQL ≠ PostgreSQL ≠ DuckDB — syntax differs (LIMIT vs TOP vs ROWNUM, date functions, string concat). **To discover vendor:** `cat /datapallas/config/connections/<slug>/<slug>.xml` and look for `<type>sqlite</type>` (or postgresql, mysql, sqlserver, oracle, duckdb). I NEVER guess the vendor — if I can\'t determine it, I ask the user. Key docs: https://datapallas.com/docs/data-exploration/database-connections | https://datapallas.com/docs/data-exploration/chat2db-ai',
        'olap-data-warehouse-analytics': 'Expert in OLAP analytics and Data Warehouse architecture. Two domains: (1) **Embeddable Analytics** — five web components (<rb-report>, <rb-tabulator>, <rb-chart>, <rb-pivottable>, <rb-parameters>) for dashboards and portals, configured via Groovy DSL; (2) **Data Warehouse Strategy** — "Start Simple, Scale as Needed" from DuckDB multi-source queries → DuckDB sync (byte-to-byte or star schema) → ClickHouse for massive scale. I help design star schemas, ETL sync jobs, and choose the right scaling level. Key docs: https://datapallas.com/docs/bi-analytics/web-components | https://datapallas.com/docs/bi-analytics/data-warehouse-olap | https://datapallas.com/docs/bi-analytics/dashboards',
        ...
        'chat2db-jupyter-interface': 'Use this skill when receiving messages from the Chat2DB/Jupyter notebook interface. This is for **RESPONDING to messages** — which may be SQL/data queries, chit-chat, or DataPallas guidance. I classify intent (DATA QUERY, CHIT-CHAT, DATAPALLAS CONFIG) and respond appropriately. When generating SQL, I investigate schema files on disk at `/datapallas/config/connections/<slug>/`. **WARNING: Schema files can be HUGE** for enterprise databases — I check file size first with `ls -lh` and grep for specific table names rather than reading whole files when they exceed 100KB.',
        'troubleshoot-cloudbeaver': 'Use ONLY when CloudBeaver is **BROKEN** — NOT for answering data queries. This is for **FIXING CloudBeaver**: (1) Database not showing in CloudBeaver; (2) Driver not found errors; (3) Authentication failures; (4) Docker/container issues; (5) Volume mount problems. I read source code, Docker configs, and workspace files to diagnose root causes. If the user wants to query data, use `chat2db-jupyter-interface` instead.',
